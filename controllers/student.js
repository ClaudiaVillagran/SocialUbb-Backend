...
const register = async (req, res) => {
  try {
    const { name, email, image, bio, password, myHashtags } = req.body;
    ...
    res.json({
      ...
      student: {
        ...
        myHashtags: newStudent.myHashtags,
        bio: newStudent.bio,
        token: access_token,
      },
    });
  } catch (error) {
    return res.status(500).send({ message: "error al registrar al usuario" });
  }
};
// const login = (req, res) => {
//   //recibir body
...
const logout = async (req, res, next) => {
  try {
    res.clearCookie("refreshtoken", { path: "/api/student/refreshtoken" });
    res.json({
      message: "logged out !",
    });
  } catch (error) {
    return res.status(404).send({ message: "error al cerrar sesión" });
  }
};
const refreshToken = async (req, res) => {
  try {
    const refresh_token = req.cookies.refreshtoken;
    if (!refresh_token) return res.status(404).send("Por favor, registrate");
    const check = await verifyToken(
      refresh_token,
      process.env.REFRESH_TOKEN_SECRET
    );
    const student = await findUser(check.studentId);
    const access_token = await generateToken(
      { studentId: student._id },
      "30d",
      process.env.ACCESS_TOKEN_SECRET
...
const profile = (req, res) => {
  ...
  Student.findById(id)
    ...
    .exec(async (error, studentProfile) => {
      ...
      const followInfo = await followService.followThisStudent(
        req.user.studentId,
        id
      );
...
const list = (req, res) => {
  ...
  Student.find()
    ...
    .sort("_id")
    .paginate(page, itemsPerPage, async (error, students, total) => {
      if (error || !students) {
        return res.status(404).send("error al encontrar usuario");
      }
...
const searchUsers = async (req, res, next) => {
  try {
    ...
    const users = await searchUsersKey(keyword, req.user.studentId);
